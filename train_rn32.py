...
model = network.cross_resnet(num_classes=num_classes,
            depth=args.depth,)
...
# According to CIFAR
module = network.Fusion_module(64, num_classes, 8)
model.to(DEVICE)
module.to(DEVICE)
...
def eval(net, module, epoch, test_flag=False):
    ...
    for batch_idx, (inputs, targets) in enumerate(loader):
        ...
        loss_kl = consistency_weight*(criterion_kl(outputs1, outputs2) + criterion_kl(outputs2, outputs1) + utils.lossOfKnowledge(criterion_kl, crossFusionKnowledge) + criterion_kl(outputs1, fused_logit) + criterion_kl(outputs2, fused_logit) +  criterion_kl(fused_logit, outputs1) + criterion_kl(fused_logit, outputs2))
